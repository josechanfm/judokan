      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="p-5 bg-white overflow-hidden shadow-xl sm:rounded-lg">
          <a-typography-title :level="3" class="text-center">{{
            competition.title_fn
          }}</a-typography-title>
          <a-typography-title :level="4"
            >{{ $t("date") }}: {{ competition.start_date }} -
            {{ competition.end_date }}</a-typography-title
          >
          <!--
            ...
            <template v-if="application.role == 'athlete' && application.gender != ''">
              <a-form-item :label="$t('category')" name="category">
                <a-radio-group v-model:value="application.category">
                  <a-radio
                    v-for="cat in competition.categories_weights"
                    :style="virticalStyle"
                    :value="cat.code"
...
export default {
  ...
  methods: {
    ...
    onFinish() {
      ...
      this.$inertia.post(
        route("member.competition.applications.store", this.competition.id),
        this.application,
        {
          onSuccess: (page) => {
            console.log(page);
