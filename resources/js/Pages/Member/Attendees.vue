  ...
  <MemberLayout title="Dashboard">
    ...
    <div class="container mx-auto pt-5">
      <div class="flex-auto h-10">
        ...
        <a href="#" onclick="history.back();return false;" class="ant-btn float-right"
          >Go Back</a
        >
      </div>
      <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-5">
        <div class="pb-2">
          <a-button @click="onClickConfirm" class="float-right mr-5">{{
            $t("confirm")
          }}</a-button>
        </div>
        <!-- 
        ...
        <div>
          <a-typography-title :level="5"
            >{{ $t("attended_members") }}:</a-typography-title
          >
          <template v-for="attendee in attendees">
            <a-checkbox
              v-model:checked="attendee.attended"
              @change="onChangeAttendee(attendee)"
            />
            {{ attendee.attendee_id }}-{{ attendee.display_name }} {{ attendee.status
            }}<br />
          </template>
        </div>
      </div>
